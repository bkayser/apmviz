...
# In Graphic Detail | Data Science and APM
...
<img class='graphic tall' src='images/CCSCvsPTFC-stats.png'/>

## Data Science and Data Visualization

<img class='graphic tall' src='images/CCSCvsPTFC-defpasses.png'/>
...
To track the mean and standard deviation, just sum the number of transactions, the total response time, and the square of the total response time.
...
```{r}
ex1 <- read_transactions('./data/timeseries/supplychain.rds', 'frontend')
ggplot(ex1$events, aes(x=value)) + xlim(0, ex1$xlimit) +
    geom_histogram(aes(fill=transaction), bins=120, position='dodge') +
    my_plot_theme +
    summary_colorscale +
    summary_linescale +
   legend_right +
...
```{r}
est_band <- dcast(ex1$plotlines, timestamp ~ measure)
g_scatter <- scatterplot(ex1)
g_scatter +
    geom_ribbon(data=est_band, inherit.aes = F,
                ...
                fill=region_fill,
                alpha=0.4) +
    geom_line(data=filter(ex1$plotlines, measure %in% c('q25', 'q75')), 
