...
* Can it be reduced by combining values into a single value?
* Or do we have to store every measurement?
...
Space required grows linear with length of history; a simple time series

>* $min(X) = min\left(min(x_1, x_2), min(x_3, x_4), ... min(x_{n-1}, x_n)\right)$
...
Keeping a large vector of buckets:

* Linear with length of history but also the granularity of the data
* Keep N buckets for each time period
...
Keeping a complete history of every measurement

Space proportional to the number of measures
